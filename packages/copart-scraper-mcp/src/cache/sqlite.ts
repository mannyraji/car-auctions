/**
 * SQLite cache for Copart data (WAL mode, TTL-based expiry)
 */
...
function resolveDefaultPath(): string {
  ...
  return path.resolve(path.dirname(here), '..', '..', 'data', 'copart.sqlite');
}
...
export class CopartSqliteCache {
  ...
  async getSoldHistory(
    cacheKey: string,
    allowStale = false
  ): Promise<CacheEntry<CopartSoldEntry[]> | null> {
    const stmt = this.db.prepare<
      [string],
      { data: string; expires_at: number; fetched_at: string }
    ...
    return { data: JSON.parse(row.data) as CopartSoldEntry[], fetched_at: row.fetched_at };
  }

  async setSoldHistory(cacheKey: string, entries: CopartSoldEntry[], ttlDays = 7): Promise<void> {
    const now = new Date().toISOString();
    ...
    this.db
      ...
      .run(cacheKey, JSON.stringify(entries), now, expiresAt);
  }
  ...
  watchlistUpdate(lotNumber: string, updates: Partial<WatchlistEntry>): void {
    const safeEntries = Object.entries(updates).filter(
      ([k]) => k !== 'lot_number' && CopartSqliteCache.WATCHLIST_COLUMNS.has(k)
    );
